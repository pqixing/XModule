...
import com.pqixing.intellij.creator.core.MVPCodeFactory;
import com.pqixing.intellij.creator.utils.AndroidUtils;
import com.pqixing.intellij.creator.utils.PluginUtils;
...
public class MVPCreatorGenerator {
    ...
    private static void genConstractFile( Project project, String classPrefix, String modulePath) {
        ...
        if(virtualFile == null) {
            ...
            PsiFile initFile = PsiFileFactory.getInstance(project).createFileFromText(
                    name, JavaFileType.INSTANCE, com.pqixing.intellij.creator.core.MVPCodeFactory.generatContract(dbDir,classPrefix));
        ...
        String mvpModelString = com.pqixing.intellij.creator.core.MVPCodeFactory.getMvpModelString();
        PsiClass mvpModelClass = factory.createClassFromText(mvpModelString, psiFile);
        // 用拼接的代码生成MvpModel interface
        String mvpPresenterlString = com.pqixing.intellij.creator.core.MVPCodeFactory.getMvpPrecenterString();
        PsiClass mvpPresenterClass = factory.createClassFromText(mvpPresenterlString, psiFile);
        // 用拼接的代码生成MvpModel interface
        String mvpViewString = com.pqixing.intellij.creator.core.MVPCodeFactory.getMvpViewString();
    ...
    private static void genModelImplFile( Project project, String classPrefix, String modulePath) {
        ...
        if(virtualFile == null) {
            // 没有就创建一个，第一次使用代码字符串创建个类
            PsiFile initFile = PsiFileFactory.getInstance(project).createFileFromText(
                    name, JavaFileType.INSTANCE, com.pqixing.intellij.creator.core.MVPCodeFactory.generatModelImpl(dbDir,classPrefix));
    ...
    private static void genPresenerImplFile( Project project, String classPrefix, String modulePath) {
        ...
        if(virtualFile == null) {
            // 没有就创建一个，第一次使用代码字符串创建个类
            PsiFile initFile = PsiFileFactory.getInstance(project).createFileFromText(
                    name, JavaFileType.INSTANCE, com.pqixing.intellij.creator.core.MVPCodeFactory.generatPrensterImpl(dbDir,classPrefix));
    ...
    private static void genActivityFile( Project project, String classPrefix, String modulePath) {
        ...
        if(virtualFile == null) {
            // 没有就创建一个，第一次使用代码字符串创建个类
            PsiFile initFile = PsiFileFactory.getInstance(project).createFileFromText(
                    name, JavaFileType.INSTANCE, com.pqixing.intellij.creator.core.MVPCodeFactory.generatActivity(actDir,classPrefix));
