...
class FastMergeTask extends GitTask {
    ...
    String onGitProject(String gitName, String gitUrl, File gitDir) {
        ...
        com.pqixing.modularization.gradle.utils.GitUtils.run("git pull", gitDir)
        def mergeResult = com.pqixing.modularization.gradle.utils.GitUtils.run("git merge origin/$branchName  --ff-only", gitDir)?.trim() ?: ""
        if (mergeResult.startsWith("fatal:")) return Keys.TIP_GIT_MERGE_FAIL
        return com.pqixing.modularization.gradle.utils.GitUtils.run("git push", gitDir)
