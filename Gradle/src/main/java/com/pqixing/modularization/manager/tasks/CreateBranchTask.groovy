...
class CreateBranchTask extends GitTask {
    ...
    String onGitProject(String gitName, String gitUrl, File gitDir) {
        ...
        return com.pqixing.modularization.gradle.utils.GitUtils.run("manager checkout origin/$baseBranchName -b $branchName", gitDir) +"\n" + com.pqixing.modularization.gradle.utils.GitUtils.run("manager push origin $branchName", gitDir) +"\n" + com.pqixing.modularization.gradle.utils.GitUtils.run("manager branch --set-upstream-to=origin/$branchName", gitDir)
    }
}
