...
//
//        com.pqixing.modularization.gradle.modularization.Print.lnm("$project.name start upload env : $mavenInfo.name artifactId: ${wrapper.artifactId}")
//
//        String errorMsg = ""
...
//        }
//        if (!com.pqixing.modularization.gradle.modularization.CheckUtils.isEmpty(dependent.dependentLose)) {
//            errorMsg = "Lose dependency, please import them before upload : ${dependent.dependentLose.toString()}"
//        }
//        String lastRelease = com.pqixing.modularization.gradle.wrapper.MavenMetadata.create(mavenInfo.maven_url, mavenInfo.groupName, mavenInfo.artifactId).release.trim()
//        if (com.pqixing.modularization.gradle.modularization.CheckUtils.isEmpty(lastRelease)) lastRelease = Keys.VERSION_DEFAULT //如果仓库没有版本，默认使用1.0.0
//
...
//        }
//        if (com.pqixing.modularization.gradle.modularization.CheckUtils.isEmpty(errorMsg) && hasUpdate()) {
//            errorMsg = "Local code is different from remote,Please update your code or Check whether it needs to be push"
//        }
//
//        if (!com.pqixing.modularization.gradle.modularization.CheckUtils.isEmpty(errorMsg)) {
//            com.pqixing.modularization.gradle.modularization.Print.lnIde("$project.name${Keys.SEPERATOR}N$Keys.SEPERATOR$errorMsg")
//
//            errorMsg = ":$project.name " + errorMsg
//            com.pqixing.modularization.gradle.modularization.Print.lnm(errorMsg)
...
//
//        if (com.pqixing.modularization.gradle.modularization.CheckUtils.isEmpty(mavenInfo.pom_version)) mavenInfo.pom_version = baseVersion
//
//        if (mavenInfo.pom_version < baseVersion) {
//            if (mavenInfo.focusUpload) mavenInfo.pom_version = lastRelease
//            else {
//                com.pqixing.modularization.gradle.modularization.Print.lnm("Pom_version error  cur ${mavenInfo.pom_version} maven : $baseVersion ")
//                com.pqixing.modularization.gradle.modularization.Print.lnIde("$project.name${Keys.SEPERATOR}N$Keys.SEPERATOR$errorMsg")
...
//
//        return gitConfig.revisionNum != com.pqixing.modularization.gradle.modularization.TextUtils.removeMark(com.pqixing.modularization.gradle.modularization.GitUtils.run("manager println -1 --pretty=format:'%H' origin/${gitConfig.branchName} ${gitConfig.rootForGit ? '' : (project.name + '/')}", gitConfig.gitDir))
//    }
//    /**
//     * 检查Docment是否需要更新
//     * @return
//     */
//    String docUpdateLog() {
//        def documentDir = com.pqixing.modularization.gradle.modularization.MavenUtils.documentDir
//
//        def gitInfo = com.pqixing.modularization.gradle.modularization.GitUtils.run("manager println -1 HEAD --oneline --pretty=format:'%H::%D'", documentDir).trim().split("::")
//        String revisionNum = ""
//        String branchName = ""
//        if (gitInfo.length > 0)
...
//        if ("%D" == branchName) {
//            branchName = com.pqixing.modularization.gradle.modularization.GitUtils.run("manager rev-parseProjectXml --abbrev-ref HEAD", documentDir).trim()
//        }
//        if (branchName != "master") return "Document manager current branch is $config.branchName , please checkout to master before upload!!!"
//
//        def remoteLog = com.pqixing.modularization.gradle.modularization.GitUtils.run("manager ls-remote ", documentDir).readLines().find { it.endsWith("/master") }
////        Print.lnf("remoteLog $remoteLog branchName $branchName revisionNum $revisionNum")
//
//        if (com.pqixing.modularization.gradle.modularization.CheckUtils.isEmpty(remoteLog)) return ""
...
//        com.pqixing.modularization.gradle.modularization.Print.lnm("uploadArchives -> pom: $pom")
//    }
//
//    @Override
//    void end() {
