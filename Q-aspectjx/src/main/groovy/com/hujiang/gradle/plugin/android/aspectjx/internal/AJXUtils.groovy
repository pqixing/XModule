...
class AJXUtils {
    ...
    static void filterJar(JarInput jarInput, VariantCache variantCache, List<String> includes, List<String> excludes,boolean excludeJarByClassMatch) {
        if (includes.isEmpty() && excludes.isEmpty()) {
            //put in cache
            variantCache.addIncludeJar(jarInput.file.absolutePath,jarInput.name)
        } else if (includes.isEmpty()) {
            boolean isExclude = isExcludeFilterMatched(jarInput.name,excludes)
            JarFile jarFile = new JarFile(jarInput.file)
            Enumeration<JarEntry> entries = jarFile.entries()
            if(excludeJarByClassMatch) while (entries.hasMoreElements()) {
            ...
            boolean isExcludeMatched = isExcludeFilterMatched(jarInput.name,excludes)
            JarFile jarFile = new JarFile(jarInput.file)
            Enumeration<JarEntry> entries = jarFile.entries()
            while (entries.hasMoreElements()) {
                JarEntry jarEntry = entries.nextElement()
                ...
                if (excludeJarByClassMatch&&isExcludeFilterMatched(tranEntryName, excludes)) {
