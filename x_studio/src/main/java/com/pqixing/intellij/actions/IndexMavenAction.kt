...
import com.pqixing.intellij.utils.GradleUtils.defEnvs
...
class IndexMavenAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        dialog.setOnOk(Runnable {
            ...
            runTask(project, listOf(":IndexMaven"),
                    ProgressExecutionMode.IN_BACKGROUND_ASYNC, true
                    , System.currentTimeMillis().toString() + "", defEnvs.toMap().plus(mapOf("opts" to excludes))
                    , TaskCallBack { s: Boolean, l: String ->
                if (!s) {
