...
open class IndexMavenTask : BaseTask() {
    ...
    private fun allBranches(): Set<String> {
        ...
        allBranches += args.manifest.fallbacks
        allBranches += args.vm.readCurVersions().keys.map { it.replace(args.manifest.groupId + ".", "").substringBefore(".") }.toSet()
        return allBranches
    }
    ...
    private fun parseVersion(excludes: List<String> = emptyList()) {
        ...
        //上传版本号到服务端
        reloadVersion(args.manifest.fullUrl(), allBranches().minus(excludes), versions)
